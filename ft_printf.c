...
{
	va_list	ap;
	// 1. 연결리스트인 `lst_pholder`와 `lst_args`를 만든다.
	t_list	*lst_pholder;
	t_list	*lst_args;

	// 2. `lst_pholder`에 자리지정자를 담는다.
	if (get_pholders(lst_pholder, str))
		return ;
	// 3. `lst_args`에 가변인자를 담는다.
	va_start(ap, str);
	va_arg(ap, int);
	va_end(ap);
	// 4. `lst_pholder`의 길이와 `lst_args`의 길이를 비교한다. `lst_pholder`의 길이가 더 길면 오류 반환
	// 5. 문자열을 `%`를 만날 때까지 출력한다.
	// 6. `%`를 만나면 다음 문자가 `%`인지 체크해서 맞을 경우 `%`처리 로직 수행(`%` 한번만 찍기)
	// 7. 자리지정자로 판단되면 `lst_pholder`와 `lst_args`로부터 하나씩 꺼내와서 데이터 처리/출력한다.
